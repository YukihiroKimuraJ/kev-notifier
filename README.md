...
- **不自然なカタログはstateへ反映しない** — 件数不一致・重複/不正なCVE ID・25件超かつ5%超の件数減少・1回100件超の追加・16 MiB超の応答を拒否し、最後に正常だったstateを保つ。
- **外部文字列はSlackのplain textで表示** — カタログの値をリンク・メンション・mrkdwnの制御構文として解釈させない。
